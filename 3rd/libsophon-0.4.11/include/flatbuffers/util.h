...
template<typename T> std::string NumToHexString(T t) {
  // clang-format off

  #ifndef FLATBUFFERS_PREFER_PRINTF
    std::stringstream ss;
    ss << std::setw(2) << std::setfill('0') << std::hex << std::uppercase << t;
    return "0x" + ss.str();
  #else // FLATBUFFERS_PREFER_PRINTF
    auto v = static_cast<long long>(t);
    return NumToStringImplWrapper(v, "0x%.*llx");
  #endif // FLATBUFFERS_PREFER_PRINTF
  // clang-format on
}

// Avoid char types used as character data.
template<> inline std::string NumToHexString<signed char>(signed char t) {
  return NumToHexString(static_cast<int>(t));
}
template<> inline std::string NumToHexString<unsigned char>(unsigned char t) {
  return NumToHexString(static_cast<int>(t));
}
