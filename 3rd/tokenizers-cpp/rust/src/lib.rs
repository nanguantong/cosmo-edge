...
impl TokenizerWrapper {
    ...
    /// Returns token id if the token encodes to exactly one id, otherwise None.
    pub fn token_to_id(&mut self, token: &str) -> Option<u32> {
        self.encode(token, false);
        if self.encode_ids.len() == 1 {
            Some(self.encode_ids[0])
        } else {
            None
        }
    }
...
extern "C" fn tokenizers_token_to_id(
    ...
    unsafe {
        let s = std::str::from_utf8(std::slice::from_raw_parts(token, len)).unwrap_or_default();
        match (*handle).token_to_id(s) {
            Some(id) => *out_id = id as i32,
            None => *out_id = -1,
        }
    }
}
