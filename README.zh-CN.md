...
CosmoEdge 是一款 C++ 边缘 AI 引擎，面向边缘设备上的生产级视频智能分析。它帮助团队从模型文件走向可运行应用：导入模型、编排流水线、接入视频源、在浏览器中查看 AI 叠加画面，并通过 MQTT 或 HTTP 推送结构化事件。

CosmoEdge 的运行时面向多路视频处理、硬件解码、OSD 渲染和低开销边缘部署设计。Python 适合研究和模型工具链；CosmoEdge 关注集成商在现场需要长期运行的服务。

CosmoEdge 不只提供推理 API 或演示脚本。它提供运行时、Web 控制台和集成路径，帮助团队部署、监控、调试和维护边缘 AI 应用。
...
CosmoEdge 可在边缘设备上运行提示词驱动的视觉模型。GroundingDINO 和 VLM 同属提示词驱动能力，但面向不同问题：
...
| CV 流水线验证 | 18/18 条 CV 流水线已按内部场景基线完成验证 |
...
| --- | ---: | ---: | ---: | ---: | --- | --- |
| 全流 YOLOv8n 检测 | 16 | 16 | 3/channel | 32-68(ms) | BM1688 | 解码、推理和 OSD 均开启；稳定高负载场景 |
| 共享解码密集 CV 任务 | 4 | 20 | 3/channel | 84-141(ms) | BM1688 | 多个场景任务共享解码流，展示任务并发能力 |
| 安全合规流水线 | 16 | 16 | 3/channel | 182-314(ms) | BM1688 | 检测 + 跟踪 + 属性/规则 + 告警，代表性安全业务流水线 |
| 提示词驱动 AI 流水线 | 8 | 8 | 0.2/channel | 3154-4128(ms) | BM1688 | 认证 `CosmoEdge-VL-Judge-0.8B`；VLM 异步节点；事件驱动路径，不参与逐帧同步 OSD |
