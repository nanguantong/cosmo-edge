...
CosmoEdge 当前版本为 `v0.1.0`。项目已经进入 `v1.0` 候选发布状态，剩余发布门槛是最后一轮回归测试。
...
CosmoEdge 正在准备 v1.0 发布，欢迎围绕以下方向贡献：
...
代码库来自面向生产的商业化开发，并通过了内部压力、流水线和回归验证。CosmoEdge 当前版本为 `v0.1.0`，项目正在完成 `v1.0` 前最后一轮回归测试。核心工作流和发布打包按生产落地设计；公共 API 和贡献者流程在 v1.0 前仍可能有少量稳定性调整。
