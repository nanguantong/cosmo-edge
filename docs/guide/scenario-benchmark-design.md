...
sampleIntervalSec: 5
videoRepeatCount: 0      # local 模式循环次数：0=无限循环（默认），1=只播一遍
...
```yaml
pass:
  # minProcessFpsPerChannel 的期望基准来自编排模板（AA_00001 节点 configObject.params.fps），
  # 由 scenario-package.js 解析提取，不应在此硬编码。
  # 判定语义：实测处理帧率（processCountPeriod / periodMs）不应低于编排设定的取帧率，
  # 低于则说明设备算力跟不上输入节流（丢帧/队列堆积），是真正的瓶颈信号。
  minProcessFpsRatio: 0.9   # 实测 FPS / 模板设定 FPS，默认不得低于 90%
  maxDiscardRate: 0.02
  maxAvgNodeLatencyMs: 80
  maxMemoryGrowthMbPerHour: 100
  maxCrashCount: 0
  minEventCount: 1

  # 仅在 rtsp-fidelity / rtsp-deterministic 模式下生效；local 模式下 packetDiscardUtilization 恒为 0
  maxPacketDiscardRate: 0.01
```

### videos.yml

描述视频源来源。提供三种 `mode`，对应不同的测试目的，而非简单的好坏之分。
...
- `targetFps`（编排设定值）仍作为 `minProcessFpsRatio` 的分母，但其语义在 local 模式下是**下限基准**（实测 fps 必然远大于它，ratio 通常 ≫ 1，该阈值恒 PASS，仅用于兜底发现"完全跑不动"的情况）。
...
# rtsp-deterministic / rtsp-fidelity 模式：RTSP 流（Camera/Add）
rtsp:
  - name: play-phone-01
    url: rtsp://192.168.1.100:8554/play-phone-01
  - name: play-phone-02
    url: rtsp://192.168.1.100:8554/play-phone-02
```
...
10. 输出 metrics.json、report.html、summary.xlsx
```
...
- `processCountPeriod / (periodMs / 1000)` 可用于推算实测处理帧率，与编排模板的设定 fps 对比（见 thresholds.yml 的 `minProcessFpsRatio`）。注意 `periodMs` 是累计窗口（从任务启动起算），不是固定周期；更稳健的做法是用相邻两次采样的 `processCount`（累计值）差分除以采样间隔，两者实测一致。
...
tools/scenario-bench/
  ...
  src/
    ...
    event-listener.js
    report-writer.js
    streamer.js              # 仅 rtsp-deterministic 模式需要
```

模块职责：

- `cosmo-client.js`：封装 CosmoEdge HTTP API。请求头携带 `mtk`/`token`，鉴权惯例参考前端 `src/web/src/utils/request.js`。
- `scenario-package.js`：解压和解析场景包。导出包为单个 JSON 文件，其中 `algorithmProcessdata`、`atomicList` 等字段值为 JSON 字符串，需二次 `JSON.parse`；取帧率基准从 `AA_00001` 节点的 `configObject.params` 中 `fps` 项提取。
...
- `event-listener.js`：对接 HTTP/MQTT 事件接收服务（见第六节事件验证，需改造或自写轻量接收器）。
- `report-writer.js`：输出 JSON、HTML、XLSX 报告。
- `streamer.js`：仅 `rtsp-deterministic` 模式需要，封装本地视频经 ffmpeg + MediaMTX 打包为 RTSP 的推流编排。
...
- 支持解析单个场景导出 JSON（处理字段值为 JSON 字符串的二次解析，提取编排设定 fps 作为吞吐基准）。
...
- 增加资源曲线图。
- 增加节点耗时排名。
- 增加 PASS/FAIL 阈值判定（基于 `minProcessFpsRatio` 等阈值与编排设定 fps 比对）。
...
- `local` 模式下 demux 以**源视频原生帧率全速推帧**（实测 ~42fps，编排设定 fps=3 未在 demux 层生效）。`minProcessFpsRatio` 在 local 模式下恒 PASS（ratio ≫ 1），判瓶颈应看并发上去后实测 fps 是否被压低、丢弃率是否上升；若需 demux 按 fps 取帧，注入 `param.videoReadFps`。
