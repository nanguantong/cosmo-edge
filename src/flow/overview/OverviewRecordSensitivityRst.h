...
class OverviewRecordSensitivityRst {
public:
    OverviewRecordSensitivityRst(const std::string& task_id, const std::string& name,
                                 size_t limit_count = 500, size_t limit_duration_ms = 30000);
    ~OverviewRecordSensitivityRst() = default;

    void OverviewRecordFrame(const MsgRecSensitity& rec_data);
...
private:
    ...
    void SaveLocalFile(const MsgRecSensitity& rec_data, bool need_clear = false) const;
    void SaveMem(const MsgRecSensitity& rec_data, bool need_clear = false);
    ...
    std::deque<MsgRecSensitity> frames_;
};

using OverviewRecordSensitivityRstPtr = std::shared_ptr<OverviewRecordSensitivityRst>;
